...
// ── Interfaces (adapted for Signal DCA Bot v2 schema) ──────────────────
...
export async function getStats(days?: number): Promise<Stats> {
  const client = await pool.connect();
  try {
    // Derive tp_fills from close_reason:
    //   Trail/TP4 → 4, TP3 → 3, TP2 → 2, tp1_hit → 1, else 0
    let query = `
      WITH enriched AS (
        SELECT *,
          CASE
    ...
    if (!row || parseInt(row.total_trades) === 0) {
      return {
        ...
        win_loss_ratio: 0, best_trade: 0, worst_trade: 0,
...
export async function getTPDistribution(): Promise<TPDistribution[]> {
  const client = await pool.connect();
  try {
    // Derive tp_fills from close_reason, then count how many reached each level
    const result = await client.query(`
      WITH enriched AS (
        SELECT
          CASE
      ...
      SELECT 1 as tp_level, COUNT(*) as count FROM enriched WHERE tp_fills >= 1
      UNION ALL
      SELECT 2 as tp_level, COUNT(*) as count FROM enriched WHERE tp_fills >= 2
      UNION ALL
      SELECT 3 as tp_level, COUNT(*) as count FROM enriched WHERE tp_fills >= 3
      UNION ALL
      SELECT 4 as tp_level, COUNT(*) as count FROM enriched WHERE tp_fills >= 4
      ORDER BY tp_level
    `);
    return result.rows;
  } finally {
...
export async function getDCADistribution(): Promise<DCADistribution[]> {
  const client = await pool.connect();
  try {
    // max_dca_reached: 0 = E1 only, 1 = DCA1, 2 = DCA2
    const result = await client.query(`
      SELECT 0 as dca_level, COUNT(*) as count FROM trades WHERE max_dca_reached = 0 AND closed_at IS NOT NULL
      UNION ALL
      SELECT 1 as dca_level, COUNT(*) as count FROM trades WHERE max_dca_reached = 1 AND closed_at IS NOT NULL
      UNION ALL
      SELECT 2 as dca_level, COUNT(*) as count FROM trades WHERE max_dca_reached >= 2 AND closed_at IS NOT NULL
      ORDER BY dca_level
    `);
    return result.rows;
  } finally {
