"""Backtest engine for MEXC futures scalping at 1:1 RR with maker entries.

Execution model (deliberately conservative):
- Entry: resting limit order (maker, 0% fee). A long limit at P placed on signal
  candle i (P must be below close_i) fills at the first candle j > i whose
  low < P (strict trade-through). Orders expire after `ttl` candles.
- TP: limit (maker, 0% fee), fills only on strict trade-through (high > TP for
  longs). SL: stop-market (taker fee + slippage applied against us).
- Intrabar ambiguity: if both TP and SL are reachable within one candle, the
  trade is counted as a LOSS. This understates real performance; anything that
  survives this rule has margin of safety.
- A time stop (`max_hold` candles) closes at candle close, taker fee + half
  slippage.

All distances are fractions of entry price. Results are reported in R (risk
units) net of all costs, so 1:1 RR gross means wins < +1R and losses < -1R net.
"""
...
TAKER_FEE = 0.0002   # 0.02% MEXC futures
MAKER_FEE = 0.0
SLIPPAGE = 0.0006  # STRESS: doubled slippage
