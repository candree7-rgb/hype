"""SL-execution-mechanics STUDY copy of engine_lighter.py (engine_lighter.py
itself is untouched). Adds sl_mode="hybrid" and per-trade trigger_idx
recording; everything else is identical.

sl_mode="hybrid": passive limit at SL exactly as in "stoplimit", but the taker
bail fires as soon as EITHER
  (a) price trades `adverse_cutoff * ATR60(at entry)` beyond the SL level
      (bail fill = cutoff level with full regime slippage — pessimistic: no
      better-than-cutoff fills even if the extreme only just touched it), OR
  (b) `sl_grace` bars elapse without a recovery fill (bail at the grace-window
      close with full regime slippage — same as "stoplimit"),
whichever comes first. Recovery-fill logic is identical to "stoplimit"
(trigger-candle recovery only via close-through; later candles via strict
extreme trade-through by sl_limit_eps).

Intrabar ordering for the hybrid, kept pessimistic:
  - On the trigger candle j itself: reaching the cutoff level requires passing
    through SL first (the cutoff is strictly beyond SL), so if the candle's
    extreme trades at/beyond the cutoff the bail is certain and takes priority
    over any close-based recovery claim.
  - On later candles k: if both the cutoff and the recovery threshold trade in
    the same candle, the order is unknowable on 1m data -> counted as a BAIL
    (pessimistic).
ATR60 is the same fractional ATR the strategy uses (rolling-60 mean of true
range / close), evaluated at entry_idx-1 — the last candle fully closed at the
moment of entry (causal).

Original engine_lighter.py docstring follows.
...
class Trade:
    ...
    tp_dist: float
    sl_dist: float
    outcome: str         # "tp" | "sl" | "ambiguous_sl" | "sl_limit" | "sl_bail" | "sl_bail_px" | "time"
    r: float             # net result in risk units
    net_frac: float      # net result as fraction of entry price
    trigger_idx: int = -1  # candle where the SL first triggered (-1 for tp/time)
...
def simulate(df: pd.DataFrame, signals: pd.DataFrame, symbol: str,
             ...
             sl_limit_eps: float = 0.0, adverse_cutoff: float = 1.0) -> Result:
    ...
    sl_mode:
      ...
      "stoplimit" — resting limit at SL price; fills at SL with 0 slippage only
                    ...
                    `sl_slippage`.
      "hybrid"    — like "stoplimit", but additionally bail as soon as price
                    trades `adverse_cutoff * ATR60(entry)` beyond SL (fill at
                    the cutoff level with full `sl_slippage`), whichever of
                    price-cutoff / grace-expiry comes first. See module
                    docstring for intrabar ordering (pessimistic).
    """
    ...
    atr_frac = None
    if sl_mode == "hybrid":
        # same fractional ATR construction as the strategy (causal true range)
        prev_close = np.concatenate(([np.nan], close[:-1]))
        tr = np.maximum(high - low,
                        np.maximum(np.abs(high - prev_close),
                                   np.abs(low - prev_close)))
        atr_frac = pd.Series(tr).rolling(60).mean().to_numpy() / close
    ...
    for sig in signals.itertuples(index=False):
        ...
        if outcome in ("sl", "ambiguous_sl"):
            ...
            if sl_mode in ("stoplimit", "hybrid"):
                ...
                bail_level = None
                if sl_mode == "hybrid":
                    a = float(atr_frac[entry_idx - 1])
                    cut = adverse_cutoff * a * p
                    bail_level = sl - cut if long else sl + cut
                # trigger candle: cutoff first (reaching it requires passing
                # SL, so the bail is path-certain), then close-recovery.
                if bail_level is not None and (
                        (long and low[trigger_idx] <= bail_level) or
                        (not long and high[trigger_idx] >= bail_level)):
                    bail_px_idx = trigger_idx
                elif (long and close[trigger_idx] > thresh) or \
                     (not long and close[trigger_idx] < thresh):
                    fill_idx = trigger_idx
                else:
                    for k in range(trigger_idx + 1, min(trigger_idx + 1 + sl_grace, n)):
                        # pessimistic: if cutoff and recovery both trade in
                        # candle k, count it as a bail.
                        if bail_level is not None and (
                                (long and low[k] <= bail_level) or
                                (not long and high[k] >= bail_level)):
                            bail_px_idx = k
                            break
                ...
                if bail_px_idx >= 0:
                    outcome = "sl_bail_px"
                    exit_idx = bail_px_idx
                    exit_price = bail_level * (1 - sl_slippage) if long \
                        else bail_level * (1 + sl_slippage)
                elif fill_idx >= 0:
                    ...
                    outcome = "sl_bail"
                    exit_idx = min(trigger_idx + sl_grace, n - 1)
                    c = close[exit_idx]
